...
from src.constants import EVENT_DELETED
...
from src.events.events import EventDeletedEvent
...
@command.route('/users/<int:user_id>/calendars/<int:calendar_id>/events/<int:id>', methods=['DELETE'])
def handle_delete(user_id, calendar_id, id):
    """
    This endpoint deletes an event and all of its recurrences.
    """
    ...
    try:
        ...
        event_published = publish_event(get_sending_channel(), event, EVENT_DELETED)
        ...
        if not event_published:
            ...
            session.commit()
            logger.info(f'Deleted event for user {user_id} on calendar id {calendar_id} with id {id}')
